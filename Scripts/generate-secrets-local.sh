...
enum Secrets {
    ...
    static let FIREBASE_APP_CHECK_TOKEN = ""  // @lourou: Remove after AppCheck/AppAttest merge
...
echo "Detecting local IP address for Local development..."
...
// For other environments, edit the .env file and run ./Scripts/generate-secrets-secure.sh

// swiftlint:disable all

/// Secrets are generated automatically for Local development
enum Secrets {
    // Configuration URLs and IDs (auto-detected)
...
if [ -f ".env" ]; then
    ...
    echo "Warning: No .env file found, only generating IP-based secrets"
fi
...
echo "Generated $SECRETS_FILE with IP: $LOCAL_IP"
echo "CONVOS_API_BASE_URL: http://$LOCAL_IP:4000/api"
echo "XMTP_CUSTOM_HOST: $LOCAL_IP"
