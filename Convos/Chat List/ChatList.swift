    ...
    private var mainContent: some View {
        VStack(spacing: 0) {
            ChatListNavigationBar(
                currentIdentity: identityStore.currentIdentity!, // swiftlint:disable:this force_unwrapping
                onIdentityTap: { showDropdownMenu = true },
                onQRTap: { print("QR tapped") },
                onWalletTap: { print("Wallet tapped") },
                onComposeTap: { print("Compose tapped") }
            )
...
#Preview {
    ...
    ChatListView(conversationStore: CTConversationStore(), identityStore: try! CTIdentityStore())
}
